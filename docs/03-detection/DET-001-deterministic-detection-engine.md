...
WP1–WP8 are implemented. WP7 is validation infrastructure in
`knowledge/validation/validate_wp7_golden_runner.py` (canonical check #17). WP8 is the production integration
boundary (`knowledge/runtime/result.py`, public `evaluate_detection_from_governed`) that assembles the final
promoted `DetectionResult` (result assembly, stage 19) with full version pinning, validated by
`knowledge/validation/validate_wp8_integration.py` as canonical check #18. The separate design gate remains
canonical check #10. WP8 is **implementation-complete locally** with the local gate 18/18 and ci_selftest 6/6
GREEN; **Phase 3 is technically ready for closure but not yet formally closed** — closure is PENDING independent
approval and the repository steps (commit / PR / remote GitHub Actions / merge / `phase3-wp8-v1.0` tag). Phase 4
is not started and G-09 remains open.
...
| 1.0.7 (WP8 integration; Phase-3 closure pending) | 2026-09-05 | P3-WP8 implemented locally (`knowledge/runtime/result.py` + `knowledge/runtime/engine.py`, gate check #18): the production `evaluate_detection_from_governed` assembles pipeline stage 19 — one immutable, JSON-Schema-valid + semantically-valid, fully-provenance-pinned `DetectionResult` (`result_contract_version` 1.1.0) from the authoritative WP5 `DecisionResult` + WP6 `ExplanationResult`, adding NO detection semantics. Support-first orchestration (only SUPPORTED/PARTIALLY_SUPPORTED execute rules; other states route through WP5 over an empty rule set — never `NO_SCAM_PATTERN` by skip); PUBLISHED-only preview exclusion; caller-immutable engine version (`ENGINE_VERSION = "1.0.0"`, runtime-owned, not a bundle member) and profile (`DEFAULT_PROFILE`, `EvaluationProfile` gains explicit `profile_id = "mvp-default"`); deterministic component-version translation (`extraction_schemas`→`extraction_contracts`, `evidence_manifest`/`evidence_records` dropped); action-policy pin reconciliation; full schema + reusable semantic + assembler reconciliation with typed `DetectionResultError`; privacy-minimised, deterministic with identity/time invariance, fail-closed on forgery. The promoted semantic invariants moved into `knowledge/runtime/result.py` (shared with `validate_runtime_contracts.py`, reusing `aggregation.RISK_MATRIX`). No promoted-schema change, no golden/rule/negative-library/action-policy change. Independent-review remediation applied: **H1** a trusted `WHOLE_EVALUATION` diagnostic now requires `input_support_status=ERROR` (any non-ERROR support with a whole-error fails closed `INVALID_INPUT_CONTEXT`; no silent normalisation); **M1** the emitted `evaluation_profile` policy ids are reconciled against the ACTUAL executed WP5 authority (`aggregation.RISK_MATRIX_ID`/`CONFIDENCE_POLICY_ID`), not merely against each other (`PROFILE_MISMATCH` on drift); **M2** `evaluation_timestamp` is validated as a real timezone-aware ISO-8601 calendar instant via `datetime.fromisoformat` (rejects calendar-impossible/naive values). Canonical gate 18/18, `ci_selftest` 6/6 (a WP8-exclusive malformed-`ENGINE_VERSION` defect), WP8 103/103. **WP8 implementation-complete locally; Phase 3 technically ready for closure but NOT yet formally closed — closure pending independent approval + commit/PR/remote CI/merge/`phase3-wp8-v1.0` tag; no accuracy claim (G-09 open); Phase 4 not started.** | Detection Architect |
