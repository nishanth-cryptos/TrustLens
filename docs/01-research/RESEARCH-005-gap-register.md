...
| **G-08** | **Zero non-English content.** Every trigger cue in all 30 rules is English; no Devanagari, Tamil, Telugu or transliterated Hinglish. | 🟡 **scoped out of MVP (2026-08-29); future work** | Multilingual **detection** is now out of MVP scope, so this no longer blocks: [CONF-004](../00-program/conflict-register.md) resolved by [OI-04](../00-program/PROGRAM-001-program-charter.md#11-open-issues)/[DEC-008](../00-program/decision-log.md) as **English-only**. | Gap persists as **future** work: widening scope needs a research pass for **verified** non-English official cues (any without it stay `HEURISTIC`, un-publishable). Non-English input is flagged `UNSUPPORTED` meanwhile (A-006, NFR-009). |
