...
|---|---|---|
| 1 | The traceability chain | 1.2 Document Conventions |
| 2 | The staged scam funnel and where TrustLens acts | 1.4 Product Scope |
| 3 | The eight product functions as a pipeline | 2.2 Product Functions |
| 4 | Control flow through rule evaluation and scoring | 4.3 Rule-Based Detection and Scoring |
| 5 | The rule lifecycle, with rejection and rollback | 4.7 Knowledge Base Governance |
| B-1 | Level 0 context diagram | Appendix B |
| B-2 | Level 1 panel A — ingest to explanation | Appendix B |
| B-3 | Level 1 panel B — adjudication and reporting | Appendix B |
| B-4 | Level 1 panel C — governance and administration | Appendix B |
| B-5 | Level 2 — process 3.0 exploded | Appendix B |

Table B-1 in Appendix B registers all eighteen boundary-crossing flows.
...
| *&lt;Name&gt;* | 2026-08-23 | Editorial and presentational revision. Ten diagrams added (Figures 1 to 5, B-1 to B-5) and Appendix B rewritten to carry them, with Table B-1 registering all eighteen boundary flows and a data-store table added. Section-sign references replaced with spelled-out section names. Reading-suggestion table restated by section name. Two corrections: ASM-009 and ASM-012 were cited by CON-007 and TBD-15 but not reproduced in Section 2.7, and are now listed there; the **shall** convention in Section 1.2 claimed one occurrence per statement, which fourteen compound statements do not meet, and now states the rule the document actually follows. No requirement was added, removed, renumbered or reworded; REQ-1 to REQ-70 and NFR-1 to NFR-28 are unchanged from v1.0. | 1.1 |
...
| Convention | Meaning |
|---|---|
| **shall** | Marks a binding obligation. Each REQ and NFR states one requirement; where that requirement has two inseparable halves — a positive obligation and its matching prohibition — both halves are written with **shall** in the same statement rather than split across two identifiers. Fourteen statements are compound in this way. |
...
| `MP Sec. n` | Section *n* of the Master Execution Prompt, the authoritative programme input. |
| Figure *n* | A diagram. Diagrams are normative where they state a flow or an ordering; where a diagram and the prose disagree, the prose governs. |
...
| **F** | Future — recorded so the architecture does not preclude it. **No requirement in release 1.0 carries this priority**; the level exists so that a later one can. |

**Traceability.** Every requirement carries an identifier and a source. The chain is:

```figure
fig-traceability
Figure 1 — The traceability chain. Every requirement in Sections 4 and 5 can be walked back to a
programme input, and forward to the test that verifies it.
```
...
|---|---|---|
| **Developer** | 2.5 Constraints · 3 External interfaces · 4 System features | 5 Nonfunctional requirements · Appendix B analysis models |
| **Tester / QA** | 4 REQ statements · 5.4 Quality attributes | 5.1 Performance targets · Appendix C |
| **Analyst / knowledge editor** (end users of the internal tooling) | 2.3 User classes · 4.5 Analyst review · 4.7 Knowledge governance | Appendix A glossary |
| **Project sponsor** | 1.4 Product scope · 2.7 Assumptions · Appendix C | 5.5 Business rules |
| **Reviewer / evaluator** | Sections 1 and 2 in full | Section 4, then Appendix C to see what is honestly still open |
...
```figure
fig-funnel
Figure 2 — The staged funnel that Indian digital fraud follows, and the point at which TrustLens
acts on it. The product analyses the artifact; it never intervenes in the channel.
```
...
schema conformance — is specified in Section 5.4 with verification methods attached.

## 1.5 References

| # | Document | Version / Date | Location |
|---|---|---|---|
| R1 | TrustLens Master Execution Prompt | Supplied, authoritative | Programme input, cited as `MP Sec. n` |
...
| R21 | TrustLens DFD set — levels 0, 1 and 2, PlantUML source | 2026-08-23 | `docs/05-architecture/diagrams/` (rendered in Appendix B) |
...
through a governed human-mediated pipeline rather than an automated feed (see Section 2.7 and
G-01 to G-04).
...
[Appendix B, Figure B-1](#appendix-b-analysis-models), and every boundary-crossing flow is
registered in Table B-1. In summary, TrustLens exchanges data with nine external entities:
...
At the highest level TrustLens performs eight functions, summarised in Figure 3. These correspond
one-to-one with the processes in the level-1 data flow diagram (Appendix B, Figures B-2 to B-4), so
the two representations can be checked against each other.
...
```figure
fig-functions
Figure 3 — The eight product functions as a pipeline. Function 7.0 supplies the rule set that 3.0
evaluates; function 8.0 spans all of them. Numbering matches the level-1 model in Appendix B.
```
...
|---|---|---|
| CON-001 | No automatic submission of legal or regulatory reports. | `MP Sec. 1`, NG-01 |
| CON-002 | No accuracy claim unsupported by reproducible evaluation on a described dataset. | `MP Sec. 17` |
| CON-003 | The rule engine is the primary decision authority; AI output is non-authoritative and advisory. | `MP Sec. 3` |
| CON-004 | No fabricated advisories, citations, statistics, regulatory obligations or datasets. | `MP Sec. 2 and 21` |
| CON-005 | Synthetic examples shall be labelled synthetic and never presented as real samples. | `MP Sec. 8 and 21` |
...
| CON-012 | Severity is an ordinal from `LOW` to `CRITICAL`. Numeric risk or confidence values shall not be stored on a rule; the research package's 0 to 100 scores are retained only as non-operational provenance. | CONF-001, R16 |
...
|---|---|---|---|
| **ASM-001** | **No end-user, analyst or official-body stakeholder is available; all personas, journeys and *DERIVED* requirements are engineering inference.** | **LOW — load-bearing** | Requirements may target the wrong users and journeys. Sections 2.3 and 4 would need rework. |
...
| ASM-009 | No fixed programme end date exists; scope is staged by dependency, not by calendar. | MEDIUM | No requirement in this document can be time-boxed ([TBD-15](#appendix-c-to-be-determined-list)). |
| ASM-011 | PostgreSQL 16 runs via Docker Compose in local development. | MEDIUM | Neither is currently installed (RSK-006). |
| ASM-012 | Delivery capacity is one engineer plus AI assistance. | HIGH | Sequencing and parallelisation change; this is the origin of CON-007. |
...
| **No labelled corpus** | None exists and none is obtainable within the programme. | Accuracy is unmeasurable (G-09, RSK-003). Constrains Section 5.4 to provable attributes only. |
...
```figure
fig-eval-flow
Figure 4 — Control flow through rule evaluation and scoring, showing both fail-safe exits. The
order is a requirement, not an implementation choice: suppressors are applied before scoring.
```
...
```figure
fig-lifecycle
Figure 5 — The rule lifecycle required by REQ-48, with the rejection path and the rollback that
REQ-51 requires. Publication is the only transition an editor cannot perform.
```
...
| **NFR-6** | The system shall route ambiguous or weakly supported cases to review or return `INSUFFICIENT_EVIDENCE` rather than presenting an uncertain conclusion as a certain one. | `MP Sec. 3`, FR-035, FR-036 |
...
| **DFD** | Data Flow Diagram — the analysis model used in Appendix B. Levels 0, 1 and 2 are drawn as Figures B-1 to B-5. |

---

# Appendix B: Analysis Models

Five data flow diagrams model the system: one context diagram, three panels that together form the
level-1 decomposition, and one level-2 explosion of process 3.0. They are balanced against each
other — every flow at level 0 reappears in exactly one level-1 panel, and every flow belonging to
process 3.0 reappears at level 2.

**Notation.** DeMarco and Yourdon. An **oval** is a process, a **rectangle** is an external entity
(terminator), and an **open rectangle** carrying its identifier in a left-hand compartment is a
data store. A **dashed outline** marks a Post-MVP element. Level 0 contains no data store, by
convention.

The same models are held in PlantUML source form at
`docs/05-architecture/diagrams/TrustLens-DFD-L0.puml`, `-L1.puml` and `-L2.puml` (R21). Where a
diagram and the requirement text disagree, the requirement text governs.

## Level 0 — context

```figure
fig-dfd-l0
Figure B-1 — Level 0 context diagram. One process, nine external entities, eighteen flows, no data
stores. Flow names are given in Table B-1.
```

Two modelling points are load-bearing:

- **F9, the report export, leaves process 0.0** — it is not drawn as a hand-off between two
  external entities. That is both a DFD legality requirement and an accurate statement of NG-01:
  the export is access-controlled and user-initiated, and nothing is submitted automatically.
- **The AI assist provider sits outside the boundary**, with schema-checked drafts returning
  inward, which is NG-07 expressed structurally rather than as prose.

**Table B-1 — Flow register.** Every flow crossing the system boundary, with the level-1 process
that owns it.

| Flow | Name | From | To | Requirement |
|---|---|---|---|---|
| F1 | Submitted artifacts | Reporting user | 1.0 | REQ-1 to REQ-5 |
| F2 | Corrections and re-evaluation request | Reporting user | 2.0 | REQ-17 |
| F3 | Verdict and evidence trace | 4.0 | Reporting user | REQ-29 to REQ-35 |
| F4 | Report bundle | 6.0 | Reporting user | REQ-41 |
| F5 | Queued case with full score decomposition | 5.0 | Analyst | REQ-37 |
| F6 | Adjudication and rationale | Analyst | 5.0 | REQ-38 |
| F7 | Configuration and retention policy | Administrator | 8.0 | REQ-58 |
| F8 | Health, audit and metrics | 8.0 | Administrator | REQ-56, REQ-69 |
| F9 | Access-controlled report export | 6.0 | Report recipient | REQ-43 |
| F10 | Published advisories | Guidance bodies | 7.0 | REQ-49, REQ-50 |
| F11 | Draft rule with source references | Knowledge editor | 7.0 | REQ-45 |
| F12 | Schema and lint verdict | 7.0 | Knowledge editor | REQ-46 |
| F13 | Review package — diff, impact analysis, regression | 7.0 | Knowledge approver | REQ-48, REQ-52 |
| F14 | Publish, reject or roll back decision | Knowledge approver | 7.0 | REQ-48, REQ-51 |
| F15 | Reputation lookup *(Post-MVP)* | 3.0 | Threat-intel providers | REQ-59 |
| F16 | Provider verdict, labelled non-authoritative *(Post-MVP)* | Threat-intel providers | 3.0 | REQ-59, REQ-63 |
| F17 | Isolated content prompt *(Post-MVP)* | 2.0 | AI assist provider | REQ-66 |
| F18 | Schema-checked draft *(Post-MVP)* | AI assist provider | 7.0 | REQ-62, REQ-64 |

Nine flows enter the system (F1, F2, F6, F7, F10, F11, F14, F16, F18) and nine leave it (F3, F4,
F5, F8, F9, F12, F13, F15, F17).

## Level 1 — decomposition

Level 1 explodes process 0.0 into eight processes and six data stores. It is drawn as three panels
so that each stays legible at page width. **The three panels are one diagram**: every level-0 flow
appears in exactly one panel, and no flow appears twice. Where a process in one panel exchanges
data with a process in another, the counterpart is drawn as a labelled reference box rather than
repeated.

```figure
fig-dfd-l1a
Figure B-2 — Level 1, panel A: ingest, normalisation, evaluation and explanation. This is the path
every submission takes, and the path that must complete when enrichment and AI assistance are off.
```

```figure
fig-dfd-l1b
Figure B-3 — Level 1, panel B: adjudication and report assembly. Process 6.0 draws on three stores,
which is what makes an identical bundle reproducible months later (REQ-42).
```

```figure
fig-dfd-l1c
Figure B-4 — Level 1, panel C: knowledge governance and administration. Nothing reaches the
rule-set store except through 7.0, and 7.0 has no path to a case.
```

| Process | Panel | Inherits level-0 flows |
|---|---|---|
| `1.0` Ingest and preserve evidence | A | F1 |
| `2.0` Normalise and extract | A | F2, F17 |
| `3.0` Evaluate rules and score | A | F15, F16 |
| `4.0` Compose explanation | A | F3 |
| `5.0` Route and adjudicate | B | F5, F6 |
| `6.0` Assemble report bundle | B | F4, F9 |
| `7.0` Govern knowledge base | C | F10, F11, F12, F13, F14, F18 |
| `8.0` Administer, audit and observe | C | F7, F8 |

| Store | Contents | Written by | Read by |
|---|---|---|---|
| `D1` Evidence Store | Artifacts with hash and chain-of-custody metadata | 1.0 | 2.0, 6.0 |
| `D2` Case Store | Cases, findings, analyst notes and adjudications | 5.0 | 6.0 |
| `D3` Rule-Set Store | Immutable published rule-set versions | 7.0 | 3.0 |
| `D4` Source Register | Graded source records and the verification manifest | 7.0 | 4.0 |
| `D5` Analysis Record | Pinned inputs, rule-set version and scores, for replay | 3.0 | 6.0 |
| `D6` Audit Log | Append-only security- and knowledge-relevant events | 5.0, 7.0, 8.0 | 8.0 |

## Level 2 — process 3.0, evaluate rules and score

```figure
fig-dfd-l2
Figure B-5 — Level 2: process 3.0 exploded into five sub-processes. The ordering carries a
requirement — 3.3 runs before 3.4.
```

`3.1` Load and pin rule set → `3.2` Match composite rules → `3.3` Apply suppressors and resolve
conflicts → `3.4` Score risk and confidence separately → `3.5` Decide outcome and route.
...
|---|---|---|---|---|
| **TBD-1** | No end user, analyst or knowledge editor has validated any persona, journey or *DERIVED* requirement. | Confidence in Sections 2.3 and 4 | Sponsor — access to even one real user | ASM-001, OI-01 |
| **TBD-2** | Data retention period and legal basis undecided; DPDP applicability unverified and no compliance claim made. | REQ-57, REQ-58, NFR-15, Section 6 | Sponsor + qualified legal review | ASM-015, OI-05 |
...
| **TBD-4** | Which Indian languages are in MVP scope. Currently English-only by default, and no verified source supplies non-English cues. | REQ-12, NFR-9, Section 6 internationalisation | Sponsor decision | CONF-004, G-08, OI-04 |
| **TBD-5** | Default evidence retention is 90 days as a placeholder, not a policy decision. | REQ-58 | Sponsor | ASM-014 |
| **TBD-6** | Deployment target unknown — local-only or a hosted environment. | Section 2.4, operational requirements | Sponsor | OI-06 |
...
| **TBD-12** | No labelled real-world corpus exists, so accuracy is unmeasurable. **Unclosable within this programme** — recorded so it is never quietly forgotten. | Section 5.4.2, any accuracy claim | Out of scope; permanent disclosure required | G-09, RSK-003 |
